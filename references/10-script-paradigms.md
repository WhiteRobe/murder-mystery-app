...
```jsonc
{
  ...
  "truth": {
    ...
    "text": "凶手是角色 X，动机是...",
    "timeline": [   // ============ 作案时间线 ============
      { "time": "23:00", "title": "凶手到访死者起居室", "text": "案发当晚 23:00 凶手实际进入死者房间……" }
    ]
  }
}
```

**两个 timeline 极易混淆，务必区分**（踩坑经验）：

| 字段 | 含义 | 何时展示 |
|---|---|---|
| 顶层 `timeline[]`（T01–T08） | **游戏流程**：自我介绍→搜证→讨论→投票等阶段，由安全服务自动注入 9 个步骤 | 玩家搜索页/进程推进、DM 步骤控制 |
| `truth.timeline[]` | **作案时间线**：案发当晚各角色**实际上**做了什么（从角色剧本【行踪】+凶手手法汇总） | DM 真相复盘「作案时间线」卡、玩家真相页 |

`truth.timeline` 每条 `{ time, title, text }`，`text` 支持角色名主题色高亮。**真相复盘/真相页的时间线必须读 `truth.timeline`，绝不能渲染顶层 `ST.timeline`**（那是游戏流程，不是案件剧情）。若漏配 `truth.timeline`，前端会回退显示游戏流程时间线并在该卡提示。数据仍由作者从剧本里整理，**纯文字即可，不要转图**。
